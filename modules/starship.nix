{ config, pkgs, lib, ... }: {
  programs.starship = {
    ...
    settings = builtins.fromTOML ''
      ...
      untracked = '[󰡯 ''${count} ](fg:red bg:#252525)'
      stashed = '[󰏗 ''${count} ](fg:#A52A2A bg:#252525)'
      modified = '[󰷉''${count} ](fg:#C8AC00 bg:#252525)'
      staged = '[''${count} ](fg:green bg:#252525)'
      renamed = '[''${count} ](fg:yellow bg:#252525)'
      deleted = '[󰆴 ''${count} ](fg:orange bg:#252525)'
      disabled = false
      ...
      [status]
      format = '[$symbol $status $hex_status( 󰜺 $signal_number-$signal_name)]($style)'
      ...
      [[battery.display]]
      threshold = 20
      style = "fg:#ff6b6b bg:#1e1e1e"

      [[battery.display]]
      threshold = 50
      style = "fg:#fcbf49 bg:#1e1e1e"

      [[battery.display]]
      threshold = 100
      style = "fg:#a0e07a bg:#1e1e1e"
